...
void CTelnetClient::HandleMessage(BYTE* pBuffer, int nRead)
{
	int       nState;
	int       nRet;
	int       nLast;
	...
	while (nRet >= 0) {
		if (nIndex == nRead) {
			...
			nRet = HandleOptions((int)pBuffer[nIndex]);
		}
		switch (nState) {
		case 0:
			if (nRet == 0) {
		...
		case 1: // char
			if (nRet != 0) {
				...
				int nLen = nIndex - nLast;
				int nPut = m_QueueRx.Put(pBuffer + nLast, nLen);
				if (nPut != nLen) {
	...
	if (bNeedNotify && IsWindow(m_hWndNotify)) {
